...
def gpt3_completion(prompt, engine='text-davinci-003', temp=0.7, top_p=1.0, tokens=300, freq_pen=0.25, pres_pen=0.05, stop=['USER:', 'BOSWORTH:']):
    max_retry = 5
    retry = 0
...
if __name__ == '__main__':
    ...
    vdb = pinecone.Index("bosworth-core")
    while True:
        ...
        timestamp = time()
        timestring = timestamp_to_datetime(timestamp)
        #message = '%s: %s - %s' % ('BOSWORTH', timestring, output)
        message = output
        vector = gpt3_embedding(message)
        unique_id = str(uuid4())
        metadata = {'speaker': 'BOSWORTH', 'time': timestamp, 'message': message, 'timestring': timestring, 'uuid': unique_id}
        save_json('nexus/%s.json' % unique_id, metadata)
        payload.append((unique_id, vector))
        vdb.upsert(payload)
        print('\n\nBOSWORTH: %s' % output) 